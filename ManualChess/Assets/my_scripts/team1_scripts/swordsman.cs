...
public class swordsman : MonoBehaviour {
	...
	public bool ThereIsNoOneAtThisPoint(float m, float k)
	{
		int sayac1 = 0;
		int sayac2 = 0;
		Vector3 currentVector = new Vector3 (m, 0, k);
	...
	public float MatrixScore(float m, float k) //tek bir noktada skor nasıl hesaplanır bunu yap sadece. Geliştirmeye en açık olan bölüm.
	{
		...
		if (ThereIsNoOneAtThisPoint(m, k)) {
			...
			//		}
			//attack
			for (int i = 0; i < Team2Units.Length; i++) { //if inrange and if the targetlerden birinin canının azlığına oranla artan bir atk değeri ve strenght karşılaştırması. Not:range işini burda karıştırmadım.
				int hpfarki = stats.hp - Team2Units [i].GetComponent<Stats> ().hp;
				atkscore += hpfarki;
				int strfarki = stats.strength - Team2Units [i].GetComponent<Stats> ().strength;
				atkscore += strfarki;
				float uzaklik = Vector3.Distance (currentVector, Team2Units [i].transform.position);
				int tileUzaklik = (int)(uzaklik / tile);
				atkscore = atkscore / (tileUzaklik/2);
			}
			//defense
	...
	public void PickDestination() //pick destination gidilebildiğini de check etmeli. gidilebiliteyi bir metod şeklinde yapabilirim belki. ya da class.
	{
		...
		for (int m = 0; m < tileX.Length; m++)
		{
			for (int k = 0; k < tileZ.Length; k++)
			{
				...
				if(max != prevMax)
				{
					Vector3 Destination = new Vector3 (tileX [m], 0, tileZ [k]); 
		...
		RealDestination = new Vector3 (tileX [em], 0, tileZ [key]);
...
//		}
	...
	{
		for (int i = 0; i < Team2Units.Length; i++) {
			if ((Team2Units [i].transform.position.x < this.transform.position.x + stats.atkRange) && (Team2Units [i].transform.position.x > this.transform.position.x - stats.atkRange)) {
				if ((Team2Units [i].transform.position.z < this.transform.position.z + 0.3f) && (Team2Units [i].transform.position.z > this.transform.position.z - 0.3f)) {
					Team2Units [i].GetComponent<Stats> ().hp -= stats.atk;
					break;
				}
			}
		}
		for (int i = 0; i < Team2Units.Length; i++) {
			if ((Team2Units [i].transform.position.z < this.transform.position.z + stats.atkRange) && (Team2Units [i].transform.position.z > this.transform.position.z - stats.atkRange)) {
				if ((Team2Units [i].transform.position.x < this.transform.position.x + 0.3f) && (Team2Units [i].transform.position.x > this.transform.position.x - 0.3f)) {
					Team2Units [i].GetComponent<Stats> ().hp -= stats.atk;
					break;
				}
			}
